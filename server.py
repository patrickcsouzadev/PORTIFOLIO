...
def send_email_via_sendgrid(nome: str, empresa: str, email: str, mensagem: str) -> bool:
    ...
    if not sendgrid_api_key:
        raise ValueError("SendGrid API Key não configurada no arquivo .env")
...
"""
    ...
    try:
        ...
        if response.status_code in [200, 202]:
            ...
            print(f"📧 De: {from_email} → Para: {recipient}")
            
            # Aviso se remetente e destinatário são iguais
            if from_email.lower() == recipient.lower():
                print("⚠️ AVISO: Remetente e destinatário são iguais!")
                print("   Outlook pode filtrar esses emails. Verifique:")
                print("   - Pasta de Spam/Lixo Eletrônico")
                print("   - Outras pastas do Outlook")
                print("   - Considere usar outro email como destinatário")
...
async def send_contact_email(request: ContactRequest):
    ...
    try:
        ...
        # Erros de configuração ou SMTP
        print(f"Erro de validação/SMTP: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
